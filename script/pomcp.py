...
class POMCP():
    ...
    # give unobservable state, action, and observation space
    ...
    # Search module
    def Search(self, cur_state: list):
        ...
        for j in range(self.timeout):
            ...
            # assign other car intention
            for i in range(len(s)):
                cur_state[i+1].g = s[i]
                
            self.Simulate(cur_state, -1, 0)
        # Get best action
        print("search end")
    ...
    def Rollout(self, s, depth):
        ...
        sample_state, _, r = self.Generator.gen(s,action)
    ...
    def Simulate(self, s, h, depth):
        ...
        sample_state, sample_observation, reward = self.Generator.gen(s, next_action)
        ...
        # Backtrack
        self.tree.nodes[h][4].append((s[1].g, s[2].g))
    ...
    def UpdateBelief(self, action, observation):
        ...
        next_node = self.getObservationNode(action_node, observation)
        
        # self.tree.nodes[-1][4] = []

        while len(self.tree.nodes[next_node][4]) < self.no_particles:
            self.tree.nodes[next_node][4].append(self.PosteriorSample(prior, action, observation))
